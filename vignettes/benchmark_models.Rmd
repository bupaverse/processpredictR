...
```{r, echo=F}
# tensorflow and reticulate need to be installed
...
# reticulate::use_miniconda0("r-reticulate") # do not touch python interpreter in global options settings
```
...
# readr::read_csv("/Users/ninaesina/Desktop/BIARU/ICPM23_RemainingTraceAndOutcomePredictionWithTransformers/data/BPI Challenge 2017.csv") -> raw_df_bpi2017
# eventdataR::traffic_fines -> raw_df_traffic_fines

# SAVE
# saveRDS(raw_df_bpi2012, "/Users/ninaesina/Desktop/BIARU/ICPM23_RemainingTraceAndOutcomePredictionWithTransformers/data/saved/raw_df_bpi2012.Rds")
# saveRDS(raw_df_bpi2017, "/Users/ninaesina/Desktop/BIARU/ICPM23_RemainingTraceAndOutcomePredictionWithTransformers/data/saved/raw_df_bpi2017.Rds")

# LOAD
eventdataR::traffic_fines -> raw_df_traffic_fines
readRDS(file = "C:/Users/Esin/Desktop/data benchmarks/raw_df_bpi2012.Rds") -> raw_df_bpi2012
readRDS(file = "C:/Users/Esin/Desktop/data benchmarks/raw_df_bpi2017.Rds") -> raw_df_bpi2017
```

Eventlog object
```{r, eval=F}
raw_df_traffic_fines %>% is.eventlog()
raw_df_bpi2012 %>% is.eventlog()
raw_df_bpi2017 %>% is.eventlog()
```

bpi2017 to activity log.
```{r, eval=F}
raw_df_bpi2017 %>%
  dplyr::rename(start = 'Start Timestamp', complete = 'Complete Timestamp') %>%
  bupaR::activitylog(case_id = 'Case ID',
                     activity_id = 'Activity',
                     resource_id = 'Resource',
                     timestamps = c("start", "complete")) -> raw_df_bpi2017
```
...
* __base (8h) w/ 1 layer__
* __base (4h) w/ 1 layer__
* __Small w/ 2 layer__
* __Medium w/ 2 layer__
* __Large w/ 2 layer__
...
# reassign loaded model after keras::load_model_tf()
# model_0_8h_traffic_fines$model <- keras::load_model_tf("checkpoints/TF/model/")
```
  
```{r}
# evaluate
model_0_8h_traffic_fines %>% evaluate(TF_test)

# predict
model_0_8h_traffic_fines %>% predict(TF_test) -> tmp

model_0_8h_traffic_fines$vocabulary$keys_x %>% unlist -> vocab

tmp %>%
  mutate(remaining_trace = strsplit(remaining_trace, " - ")) %>% pull(remaining_trace) %>% purrr::map(~match(.x, vocab)) -> remaining_trace_toks
tmp %>%
  mutate(pred_remaining_trace = strsplit(pred_remaining_trace, " - ")) %>% pull(pred_remaining_trace) %>% purrr::map(~match(.x, vocab)) -> pred_remaining_trace_toks

stringdist::seq_dist(remaining_trace_toks, pred_remaining_trace_toks) -> lv_dist
stringdist::seq_dist(remaining_trace_toks, pred_remaining_trace_toks, method = "dl") -> dl_dist
...
```{r, eval=T, echo=T, message=F}
ex_traffic_fines %>% create_model(num_heads = 8, output_dim = 128, dim_ff = 512, num_layers = 1) -> model_0_8h_M_traffic_fines
model_0_8h_M_traffic_fines
```
...
compile(model_0_8h_M_traffic_fines) # Medium
...
# Reassign for ease of readability
bpi2012_train <- train_test_bpi2012$train_df
...
```{r, eval=T, echo=T, message=F}
ex_bpi2012 %>% create_model(num_heads = 4, output_dim = 36, dim_ff = 64, num_layers = 1) -> model_0_4h_bpi2012
model_0_4h_bpi2012
```
...
```{r, eval=T, echo=T, message=F}
ex_bpi2012 %>% create_model(num_heads = 8, output_dim = 36, dim_ff = 64, num_layers = 2) -> model_0_8h_2l_bpi2012
model_0_8h_2l_bpi2012
```
...
```{r, eval=T, echo=T, message=F}
ex_bpi2012 %>% create_model(num_heads = 8, output_dim = 128, dim_ff = 512, num_layers = 1) -> model_0_8h_M_bpi2012
model_0_8h_M_bpi2012
```

* Medium config (M) with 2 layers:
  + __num_layers = 2__
  + num_heads = 8
  + __d_model = 128__
  + __dff = 512__
...
```{r, eval=T, echo=T, message=F}
ex_bpi2012 %>% create_model(num_heads = 8, output_dim = 128, dim_ff = 512, num_layers = 2) -> model_0_8h_2l_M_bpi2012
model_0_8h_2l_M_bpi2012
```
...
<!-- # ```{r, eval=T, echo=T, message=F} -->
<!-- # ex_bpi2012 %>% create_model(num_heads = 8, output_dim = 512, dim_ff = 2048, num_layers = 1) -> model_0_8h_2l_L_bpi2012 -->
<!-- # model_0_8h_2l_L_bpi2012 -->
<!-- # ``` -->

```{r, eval=T, echo=T, message=F}
ex_bpi2012 %>% create_model(num_heads = 16, output_dim = 512, dim_ff = 2048, num_layers = 1) -> model_0_16h_L_bpi2012
model_0_16h_L_bpi2012
```
...
chckpnt <- function(model) {
  keras::callback_model_checkpoint(filepath = paste0("checkpoints/", model$model$name))
...
```{r, eval=F}
# Traffic fines
histTF_0_8h_TF <- fit(object = model_0_8h_traffic_fines, train_data = TF_train, epochs = 50,
             callbacks = list(early_stopping, chckpnt(model_0_8h_traffic_fines)))
histTF_0_4h_TF <- fit(object = model_0_4h_traffic_fines, train_data = TF_train, epochs = 50,
             callbacks = list(early_stopping, chckpnt(model_0_4h_traffic_fines)))
histTF_0_8h_M_TF <- fit(object = model_0_8h_M_traffic_fines, train_data = TF_train, epochs = 50,
             callbacks = list(early_stopping, chckpnt(model_0_8h_M_traffic_fines)))
histTF_0_8h_2l_TF <- fit(object = model_0_8h_2l_traffic_fines, train_data = TF_train, epochs = 50,
             callbacks = list(early_stopping, chckpnt(model_0_8h_2l_traffic_fines)))
```

```{r}
# bpi2012
hist_0_8h_bpi2012 <- fit(object = model_0_8h_bpi2012,
                         train_data = bpi2012_train,
                         epochs = 15,
                         batch_size = 500,
                         callbacks = list(early_stopping, chckpnt(model_0_8h_bpi2012)))
hist_0_4h_bpi2012 <- fit(object = model_0_4h_bpi2012,
                         train_data = bpi2012_train,
                         epochs = 20,
                         batch_size = 15,
                         callbacks = list(early_stopping, chckpnt(model_0_4h_bpi2012)))
```
...
hist_0_8h_M_bpi2012 <- fit(object = model_0_8h_M_bpi2012, 
                           train_data = bpi2012_train,
                           epochs = 20,
                           batch_size = 500,
                           callbacks = list(early_stopping, chckpnt(model_0_8h_M_bpi2012)))
# hist_0_8h_2l_bpi2012 <- fit(object = model_0_8h_2l_bpi2012, 
#                             train_data = bpi2012_train, 
#                             epochs = 20,
#                             batch_size = 2000,
#                             callbacks = list(early_stopping, chckpnt(model_0_8h_2l_bpi2012)))
hist_0_8h_2l_M_bpi2012 <- fit(object = model_0_8h_2l_M_bpi2012, 
                              train_data = bpi2012_train,
                              epochs = 20,
                              batch_size = 500,
                              callbacks = list(early_stopping, chckpnt(model_0_8h_2l_M_bpi2012)))
hist_0_16h_L_bpi2012 <- fit(object = model_0_16h_L_bpi2012, 
                              train_data = bpi2012_train,
                              epochs = 20,
                              batch_size = 500,
                              callbacks = list(early_stopping, chckpnt(model_0_16h_L_bpi2012)))
```
...
Transform dataset into 'prefix' dataset using `prepare_examples()`. The first 6 rows are shown below.
```{r}
processpredictR::prepare_examples(raw_df_bpi2017, task = "remaining_trace_s2s") -> ex_bpi2017
tmpbpi2017 <- ex_bpi2017
```

Shuffling the cases.
```{r, eval=F}
# Generate a random permutation of unique ith_case values
new_group_ids <- sample(unique(ex_bpi2017$ith_case))

# Create a lookup table to map the original ith_case values to the new group identifiers
lookup_table <- data.frame(ith_case = unique(ex_bpi2017$ith_case), new_group_id = new_group_ids)

# Merge the new group identifier back into the original dataset
ex_bpi2017 <- left_join(ex_bpi2017, lookup_table, by = "ith_case")

# Sort the dataset by the new group identifier and start_time
ex_bpi2017 <- arrange(ex_bpi2017, new_group_id, start_time)

# Remove the new_group_id column if desired
ex_bpi2017 <- select(ex_bpi2017, -new_group_id)
```
...
```{r}
# Split the full 'prefix' dataset into train (0.75) and test (0.25) datasets
ex_bpi2017 %>% split_train_test(split = 0.75) -> train_test_bpi2017

# # Split the train dataset into train (0.75) and validation (0.25) datasets
# train_test_bpi2017$train_df %>% split_train_test(split = 0.75) -> train_validation_bpi2017

# Reassign for ease of readability
bpi2017_train <- train_test_bpi2017$train_df
# bpi2017_valid <- train_validation_bpi2017$test_df
bpi2017_test <- train_test_bpi2017$test_df
```

Trace analysis.
```{r, echo=F, message=F}
cat("Number of unique prefixes:", "\n", 
             "Train set - ", bpi2017_train$prefix %>% n_distinct(), "\n",
             "Test set - ", bpi2017_test$prefix %>% n_distinct(),
    sep = ""
      )

list(bpi2017_train=bpi2017_train, bpi2017_test=bpi2017_test) %>% 
  map(~
  group_by(.x, `Case ID`) %>% 
  filter(k == max(k)) %>% # keep only the fullest prefix for each case (= trace)
  ungroup() %>%
  distinct(prefix)) -> list_unique_traces_bpi2017_train_test

cat("\nNumber of unique traces:",
    "\nTrain set - ", list_unique_traces_bpi2017_train_test$bpi2017_train %>% nrow,
    "\nTest set - ", list_unique_traces_bpi2017_train_test$bpi2017_test %>% nrow,
    "\nTotal traces - ", list_unique_traces_bpi2017_train_test$bpi2017_train %>%
      mutate(set = "train") %>%
      full_join(list_unique_traces_bpi2017_train_test$bpi2017_test) %>% nrow,
    sep = "")

list_unique_traces_bpi2017_train_test$bpi2017_train %>%
  mutate(set = "train") %>%
  full_join(list_unique_traces_bpi2017_train_test$bpi2017_test %>% 
               mutate(set = "test"), by = "prefix") %>% 
  mutate(set.x = if_else(is.na(set.x), "", set.x), 
         set.y = if_else(is.na(set.y), "", set.y)) %>%
  unite(col = set, c("set.x", "set.y"), sep = "") %>% 
...
list_unique_traces_bpi2017_train_test$bpi2017_train %>%
  ...
  mutate(set.x = if_else(is.na(set.x), "", set.x), 
         set.y = if_else(is.na(set.y), "", set.y)) %>%
  unite(col = set, c("set.x", "set.y"), sep = "") %>% slice(2) %>%
  pull(prefix) %in% bpi2017_train$prefix
```
...
```{r, eval=T, echo=T, message=F}
ex_bpi2017 %>% create_model(num_heads = 8, output_dim = 36, dim_ff = 64, num_layers = 1) -> model_0_8h_bpi2017
model_0_8h_bpi2017
```
...
```{r, eval=T}
# 1 layer (Base-Small) with varying number of attention heads
compile(model_0_8h_bpi2017)
# compile(model_0_4h_bpi2017)

# # 2 layers
# compile(model_0_8h_2l_bpi2017) # Small
# compile(model_0_8h_2l_M_bpi2017) # Medium
# compile(model_0_8h_2l_L_bpi2017) # Large
```

Train models.
Model bpi2017_0_8h.
```{r, eval=F}
hist_bpi2017_0_8h <- fit(object = model_0_8h_bpi2017, train_data = bpi2017_train, epochs = 50,
             callbacks = list(early_stopping))
```
