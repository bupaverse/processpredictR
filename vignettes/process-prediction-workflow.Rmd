...
```{r}
df <- prepare_examples_dt(traffic_fines, task = "outcome")
df
```
...
df_next_time <- traffic_fines %>% 
  ...
  ungroup_eventlog() %>%
  prepare_examples_dt(task = "next_time", features = "month") %>% split_train_test()
...
```{r}
df <- prepare_examples_dt(traffic_fines, task = "next_activity") %>% split_train_test()
