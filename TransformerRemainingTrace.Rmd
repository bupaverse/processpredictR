...
vocabulary$keys_x <- vocabulary$keys_x %>% append(list("endpoint"))
...
dropout_rate <- 0.1
vocab_size <- 9
maxlen <- 7 # $max_case_length

x <- inputs <- keras::layer_input(shape = c(7))
...
x <- inputs %>%
  ...
  DecoderLayer(context=context, d_model = d_model, num_heads = num_heads, dff = dff, dropout_rate = dropout_rate) #%>% 
  keras::layer_dense(vocab_size, activation = "linear") # or maxlen?

keras::keras_model(inputs, context)
```
...
```{r}
# halfEncoder <- function() {
#   keras::new_layer_class(
#     classname = "halfEncoder",
#     initialize = function(self, d_model, num_heads, #num_layers,
#                           dff, vocab_size, maxlen, dropout_rate = 0.1, ...) { # dff = ff_dim, d_model = embed_dim, dropout_rate = rate
#       super$initialize()
#       
#       self$d_model <- d_model
...
#       self$vocab_size <- vocab_size
#       self$dff <- dff
#       self$maxlen <- maxlen ############# AANPASSEN AAN MAX_CASE_LENGTH
#       #self$num_layers <- num_layers
# 
#       #self$pos_embedding <- PositionalEmbedding(vocab_size = vocab_size, d_model = d_model)
#       self$pos_embedding <- TokenAndPositionEmbedding()(maxlen = maxlen, vocab_size = vocab_size, d_model = d_model)
#       
#       # Only one layer stack
#       self$self_attention <- GlobalSelfAttention(num_heads = num_heads,
#                                                  key_dim = d_model, 
...
#       )
#       
#       self$dropout <- keras::layer_dropout(rate = dropout_rate)
#     },
#     
#     call = function(self, x) {
#       # `x` is token-IDs shape: (batch, seq_len)
...
# 
# inputs %>% 
#   halfEncoder()(d_model, num_heads, #num_layers,
#                           dff, vocab_size, maxlen, dropout_rate = 0.1) -> a
# 
# keras::keras_model(inputs, a)
