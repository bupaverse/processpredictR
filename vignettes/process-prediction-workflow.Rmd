...
df_next_time <- patients %>% prepare_examples_dt(task = "next_time", features = "employee") %>% split_train_test()
...
```{r}
df <- prepare_examples_dt(patients, task = "next_activity") %>% split_train_test()
...
# Before we can train the model we first must prepare the data
# the trace of activities must be tokenized
tokens <- df$train_df %>% tokenize()
map(tokens, head) # the output of tokens is a list

tokens$token_x %>% pad_sequences(maxlen = max_case_length(df$train_df), value = 0) # make sequences of equal length

#fit(object = custom_model, ) # see also ?keras::fit.keras.engine.training.Model
...
In case if you want to use the provided methods for __fit()__, __predict()__ and __evaluate()__ or have access to the specifications of a event log (ex. num_outputs, max_case_length, etc. ) through the model use __map_attributes()__.
```{r}
