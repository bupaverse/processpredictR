...
#' @param output_dim Dimension of the dense embedding of the [keras::layer_embedding()].
...
#'
#' @export
create_model <- function(x_train, custom = FALSE, num_heads = 4, output_dim = 36, dim_ff = 64, ...) {
  UseMethod("create_model")
}

#' @export
create_model.ppred_examples_df <- function(x_train, custom = FALSE, num_heads = 4, output_dim = 36, dim_ff = 64, ...) { #num_heads, d_model, dff

  if (attr(x_train, "task") != "remaining_trace_s2s") {
    create_model_original(x_train, custom = custom, num_heads = num_heads, embed_dim = output_dim, ff_dim = dim_ff, ...)
  }
    ...
    create_model_s2s(x_train, num_heads = num_heads, d_model = output_dim, dff = dim_ff, ...)
  }
}
